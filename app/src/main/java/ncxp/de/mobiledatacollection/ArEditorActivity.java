...
public class ArEditorActivity extends AppCompatActivity implements ArInteractionListener {
	...
	private void initPlacing() {
		...
		ViewRenderable.builder().setView(this, R.layout.rotation_widget_controls).build().thenAccept(viewRenderable -> {
			...
			rotateRightButton.setOnTouchListener((view, motionEvent) -> rotateNode(viewModel.getCurrentSelectedNode().getValue(), 1f));
			rotateLeftButton.setOnTouchListener((view, motionEvent) -> rotateNode(viewModel.getCurrentSelectedNode().getValue(), -1f));
		});
		ViewRenderable.builder().setView(this, R.layout.scale_widget_controls).build().thenAccept(viewRenderable -> {
			...
			scaleUpButton.setOnTouchListener((view, motionEvent) -> scaleNode(viewModel.getCurrentSelectedNode().getValue(), 1.1f));
			scaleDownButton.setOnTouchListener((view, motionEvent) -> scaleNode(viewModel.getCurrentSelectedNode().getValue(), 0.9f));
